...
fetch(`${API_URL}/api/getPegawai`).then((e => e.json())).then((e => {
  console.log("Initializing application"), labels.push(...e), initializeFaceAPI()
})).catch((e => console.error("Error fetching data", e)));

function initializeFaceAPI() {
  Promise.all([
    faceapi.nets.ssdMobilenetv1.loadFromUri(`${API_URL}/models`),
    faceapi.nets.faceRecognitionNet.loadFromUri(`${API_URL}/models`),
    faceapi.nets.faceLandmark68Net.loadFromUri(`${API_URL}/models`)]).then((() => {
...
async function getImagePathsForLabel(e) {
  try {
    const t = await fetch(`${API_URL}/api/pegawai-images/${encodeURIComponent(e)}`);
...
async function saveImageToServer(e, t) {
  ...
  try {
    const e = await fetch(`${API_URL}/api/saveImage`, {
      method: "POST",
      headers: {
...
async function getPegawaiDataByLabel(e) {
  try {
    const t = await fetch(`${API_URL}/api/getPegawaiData/${encodeURIComponent(e)}`);
    if (!t.ok) throw new Error("Network response was not ok");
    return await t.json()
  } catch (e) {
...
async function saveAttendance(employeeCode, employeeNIK) {
  try {
    const attendanceCheckResponse = await fetch(`${API_URL}/check-attendance`, {
      method: "POST",
      headers: {
    ...
    if (attendanceCheckData.hasClockedIn) {
      ...
      const clockOutResponse = await fetch(`${API_URL}/store-attendance-out`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": csrfToken
      ...
      const clockInResponse = await fetch(`${API_URL}/store-attendance`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": csrfToken
    ...
    // insert to api
    const mainServerResponse = await fetch(`${API_URL}/api/proxy/server/attendance`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": csrfToken
