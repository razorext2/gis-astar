...
fetch("attendance/api/getPegawai").then((e => e.json())).then((e => {
  console.log("Initializing application"), labels.push(...e), initializeFaceAPI()
})).catch((e => console.error("Error fetching data", e)));

function initializeFaceAPI() {
  Promise.all([
    faceapi.nets.ssdMobilenetv1.loadFromUri("attendance/models"),
    faceapi.nets.faceRecognitionNet.loadFromUri("attendance/models"),
    faceapi.nets.faceLandmark68Net.loadFromUri("attendance/models")]).then((() => {
...
async function getImagePathsForLabel(e) {
  try {
    const t = await fetch(`attendance/api/pegawai-images/${encodeURIComponent(e)}`);
...
async function saveImageToServer(e, t) {
  ...
  try {
    const e = await fetch("attendance/api/saveImage", {
      method: "POST",
      headers: {
...
async function getPegawaiDataByLabel(e) {
  try {
    const t = await fetch(`attendance/api/getPegawaiData/${encodeURIComponent(e)}`);
    if (!t.ok) throw new Error("Network response was not ok");
    return await t.json()
  } catch (e) {
...
async function saveAttendance(employeeCode, employeeNIK) {
  try {
    const attendanceCheckResponse = await fetch("attendance/check-attendance", {
      method: "POST",
      headers: {
    ...
    if (attendanceCheckData.hasClockedIn) {
      ...
      const clockOutResponse = await fetch("attendance/store-attendance-out", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": csrfToken
      ...
      const clockInResponse = await fetch("attendance/store-attendance", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": csrfToken
    ...
    // insert to api
    const mainServerResponse = await fetch("attendance/api/proxy/server/attendance", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": csrfToken
