...
export function searchPegawaiHandler() {
    ...
    $("#full_name").on("input", function () {
        ...
        debounceTimer = setTimeout(() => {
            ...
            if (query.length > 0) {
                axios
                    .get(pegawaiSearch, { params: { query: query } })
                    .then((response) => {
                  ...
                  <div class="autocomplete-result bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full ${roundedClass}" data-fullname="${pegawai.full_name}" data-id="${pegawai.kode_pegawai}">
                    ${pegawai.full_name}
                  </div>
                `);
                            });
                            ...
                            $("#autocomplete-pegawai-results").append(
                                '<div class="bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full rounded-b-lg">No results found</div>',
                            );
                        }
                    })
                    .catch((error) => {
                        ...
                        $("#autocomplete-pegawai-results").append(
                            '<div class="bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full rounded-b-lg">Error loading results</div>',
                        );
                    });
            } else {
                $("#autocomplete-pegawai-results").empty();
...
export function searchSRHandler() {
    ...
    $("#no_sr").on("input", function () {
        ...
        debounceTimer = setTimeout(() => {
            ...
            if (query.length > 1) {
                axios
                    .get(srSearch, { params: { query: query } })
                    .then((response) => {
                        const data = response.data;
                        $("#autocomplete-collect-idy-ppn-container").empty();
                            ...
                            data.forEach((collectTask, index) => {
                                let roundedClass =
                                    index === data.length - 1
                                        ? "rounded-b-lg"
                                        : "";
                                $("#autocomplete-collect-idy-ppn-container")
                                    .append(`
                  <div class="autocomplete-collect-idy-ppn-item bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full ${roundedClass}" data-nosr="${collectTask.no_sr}" data-id="${collectTask.id}">
                    ${collectTask.no_sr} | ${collectTask.customer_name}
                  </div>
                `);
                            });
                        } else {
                            $("#autocomplete-collect-idy-ppn-container").append(
                                '<div class="bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full rounded-b-lg">No results found</div>',
                            );
                        }
                    })
    ...
    $(document).on("click", ".autocomplete-collect-idy-ppn-item", function () {
        let id = $(this).data("id");
