export function searchDataHandler() {
    ...
    $("#name").on("input", function () {
        ...
        debounceTimer = setTimeout(() => {
            ...
            if (query.length > 2) {
                // Mulai pencarian saat input lebih dari 2 karakter
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.length > 0) {
                            data.forEach((pegawai, index) => {
                                ...
                                <div class="autocomplete-result bg-white border border-zinc-200 dark:bg-white p-2.5 divide-y w-full ${roundedClass}" data-fullname="${pegawai.full_name}" data-id="${pegawai.kode_pegawai}">${pegawai.full_name}</div>
                            `);
                            });
                        } else {
                            $("#autocomplete-results").append(
