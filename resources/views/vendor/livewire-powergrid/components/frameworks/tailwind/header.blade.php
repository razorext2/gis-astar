<div>
        ...
        <div class="flex w-full flex-row flex-wrap items-center gap-1">
            ...
            <div class="flex flex-row flex-wrap items-center gap-2 text-sm">
                @if (data_get($setUp, 'exportable'))
                    <div class="mt-2 sm:mt-0" id="pg-header-export">
                ...
                @includeIf(data_get($theme, 'root') . '.header.refresh')
    ...
    @includeWhen(data_get($setUp, 'exportable.batchExport.queues', 0),
        data_get($theme, 'root') . '.header.batch-exporting')
