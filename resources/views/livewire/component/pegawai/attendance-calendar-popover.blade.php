<div id="popover-click-{{ $date }}" data-popover role="tooltip"
    class="invisible absolute z-[100] inline-block w-80 rounded-2xl border border-white/30 bg-white/95 p-4 text-sm shadow-2xl backdrop-blur-xl opacity-0 transition-opacity duration-300 dark:border-white/10 dark:bg-zinc-900/95 overflow-hidden">
    
    <div class="space-y-4">
        {{-- Attendance In Section --}}
        <div>
            <div class="mb-2 flex items-center gap-2 border-b border-gray-200 pb-1 dark:border-white/5">
                <div class="h-2 w-2 rounded-full bg-green-500"></div>
                <h4 class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Absensi Masuk</h4>
            </div>
                    ...
                    <table class="w-full text-left text-[11px]">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            @foreach($attendance as $item)
                                ...
                                @endphp
                                <tr class="bg-white hover:bg-gray-50 transition-colors dark:bg-transparent">
                                    <td class="px-3 py-2">
                                        <div class="flex flex-col gap-1">
                                            <span class="w-fit rounded-lg px-2 py-0.5 text-[9px] font-bold uppercase {{ $status['class'] }}">
                                                {{ $status['label'] }}
                                            </span>
                                            <span class="font-bold text-gray-800 dark:text-white">
                                                {{ \Carbon\Carbon::parse($item->jam_masuk)->isoFormat('HH:mm:ss') }}
                                            </span>
                                            <p class="line-clamp-2 text-gray-500 italic">{{ $item->keterangan }}</p>
                                            <a href="https://www.google.com/maps/search/?api=1&query={{ $item->latitude }},{{ $item->longitude }}" 
                                               ...
                                               class="flex items-center gap-1 text-blue-500 hover:underline">
                                                ...
                                                <span>{{ round($item->latitude, 4) }}, {{ round($item->longitude, 4) }}</span>
                                            </a>
                                        </div>
                                    </td>
                                    <td class="px-3 py-2">
                                        <img src="{{ $path }}" 
                                             ...
                                             class="h-12 w-12 rounded-lg object-cover shadow-sm bg-gray-100" 
                                             alt="Foto">
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div>
            <div class="mb-2 flex items-center gap-2 border-b border-gray-200 pb-1 dark:border-white/5">
                ...
                <h4 class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Absensi Keluar</h4>
            </div>
