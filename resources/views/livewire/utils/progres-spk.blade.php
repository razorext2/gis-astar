<div
    class="{{ $data->on_delay || $data->status_approval === 4 ? 'overflow-hidden' : 'overflow-x-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700' }} relative flex w-full flex-row items-center gap-2 rounded-2xl border border-zinc-100 bg-white p-4 shadow-sm transition-all dark:border-zinc-800 dark:bg-zinc-900/50 sm:p-6">

    {{-- Timeline items container --}}
    <div class="{{ $data->on_delay || $data->status_approval === 4 ? 'blur-[2px] saturate-50' : '' }} flex w-full flex-row items-center gap-4">
    ...
    @if ($data->on_delay && $data->status_approval !== 4)
        <div class="absolute inset-0 z-20 flex items-center justify-center bg-red-950/40 p-4 backdrop-blur-[4px]">
            <div class="w-full max-w-sm transform rounded-2xl border border-red-500/30 bg-white p-6 shadow-2xl transition-all dark:bg-zinc-900">
                <div class="mb-4 flex flex-col items-center">
                    ...
                    <h4 class="text-sm font-black uppercase tracking-widest text-red-600 dark:text-red-400">SPK MENGALAMI DELAY</h4>
                    <p class="mt-1 text-center text-xs font-bold text-zinc-400 dark:text-zinc-500">{{ $data->on_delay_at }}</p>
                </div>
                <div class="space-y-2 rounded-xl bg-red-50 p-4 dark:bg-red-950/20">
                    ...
                    <div class="mt-2 flex items-center justify-center gap-1 text-[10px] font-bold text-red-700/60 dark:text-red-400/50">
                        <span>BY:</span>
    ...
    @if ($data->status_approval === 4)
        <div class="absolute inset-0 z-20 flex items-center justify-center bg-zinc-950/60 p-4 backdrop-blur-[6px]">
            <div class="w-full max-w-sm transform rounded-2xl border border-red-500/30 bg-white p-6 shadow-2xl transition-all dark:bg-zinc-900">
                <div class="mb-4 flex flex-col items-center">
                    <div class="mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-red-600 text-white shadow-xl shadow-red-500/20">
                ...
                <div class="space-y-3 rounded-xl border border-red-100 bg-red-50/50 p-4 dark:border-red-900/20 dark:bg-red-950/10">
                    <p class="text-center text-sm font-semibold italic text-red-900 dark:text-red-200">
                        "{{ $data->cancel_request_reason }}"
                    </p>
                    <div class="mt-2 flex items-center justify-center gap-1.5 border-t border-red-200/50 pt-2 text-[10px] font-black tracking-widest text-red-700/60 dark:border-red-900/30 dark:text-red-400/50">
