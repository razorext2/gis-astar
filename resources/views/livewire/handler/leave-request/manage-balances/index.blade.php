...
<div class="flex flex-col gap-4" x-data="{ activeTab: 'balances' }">
    ...
    {{-- Header / Tab Switcher --}}
    <div class="flex flex-col gap-4 rounded-xl border border-zinc-200 p-4 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-between md:p-6"
            ...
            <div>
                ...
                <div class="mt-1 flex items-center gap-2">
                    <button @click="activeTab = 'balances'"
                        :class="activeTab === 'balances' ? 'text-red-600 bg-red-50 dark:bg-red-900/20' :
                            'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'"
                        class="rounded-lg px-3 py-1 text-xs font-bold transition-all">
                        Kelola Saldo
                    </button>
                    <button @click="activeTab = 'types'"
                        :class="activeTab === 'types' ? 'text-red-600 bg-red-50 dark:bg-red-900/20' :
                            'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'"
                        class="rounded-lg px-3 py-1 text-xs font-bold transition-all">
                        Tipe Cuti
                    </button>
                    <button @click="activeTab = 'current'"
                        :class="activeTab === 'current' ? 'text-red-600 bg-red-50 dark:bg-red-900/20' :
                            'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'"
                        class="rounded-lg px-3 py-1 text-xs font-bold transition-all">
                        Sedang Cuti
                    </button>
