<div class="flex flex-col">
    ...
    @if ($step == 1)
        <form wire:submit.prevent="process" class="mt-4 grid gap-4 lg:grid-cols-2">
            ...
            <div class="col-span-2 flex flex-col lg:col-span-1">
                <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="end_period">Periode
                    akhir:</label>
                <input
                    class="block w-full rounded-lg border border-zinc-200 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-zinc-800 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
                ...
                <div class="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/65 p-4 backdrop-blur-sm">
                    ...
                    <div class="flex max-w-lg flex-col gap-2 rounded-xl border border-zinc-200 bg-white/60 p-6 shadow-2xl backdrop-blur-md dark:border-zinc-800 dark:bg-dark-primary/60">
                        ...
                        <p
                            ...
                            Apakah anda yakin ingin melakukan redeem poin untuk semua teknisi dengan poin valid dari
                            tanggal
    ...
    @if ($step == 3)
        ...
        @if ($results->isNotEmpty())
            <div
                class="mt-2 flex flex-col rounded-lg bg-gray-100 p-2 text-gray-800 dark:bg-gray-700 dark:text-white lg:p-4">
                <div class="flex items-center justify-between">
                    <span class="font-semibold">No. Transaksi</span>
                    <span
                        class="text-right">{{ $results->first()->transaction_id ?? 'Transaksi tidak ditemukan' }}</span>
                </div>
                <div class="flex items-center justify-between">
                ...
                <div class="pl-4">
                    @foreach ($results as $item)
                        <div class="flex justify-between">
                            <span> ({{ $item->kode_pegawai }})
                                {{ $item->pegawai->full_name ?? 'Pegawai belum terdaftar disistem' }}</span>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span
                        class="{{ $statusData['color'] }} rounded-md px-2 py-0.5 text-right">{{ $statusData['label'] }}</span>
                </div>
            </div>
