	...
	<div wire:show="showModal" wire:transition
		...
		@if ($showModal)
			...
			<div
				...
				<h2 class="mb-4 text-center text-xl font-semibold text-gray-900 dark:text-white lg:text-2xl">Preview Data</h2>

				<div class="flex max-h-96 w-full flex-col gap-2 overflow-y-auto text-gray-800 dark:text-white">
					@foreach ($data as $row)
						<table class="my-2 w-full border-spacing-4 border-y border-gray-600">
							<tr class="p-2">
								<td>Periode</td>
								<td>:</td>
								<td colspan="2" class="text-right"> {{ Carbon\Carbon::parse($row->from_date)->isoFormat('MMM YYYY') }} -
									{{ Carbon\Carbon::parse($row->to_date)->isoFormat('MMM YYYY') }}</td>
							</tr>
							<tr>
								<td>Nama Teknisi</td>
								<td>:</td>
								<td colspan="2" class="text-right"> {{ $row->pegawai->full_name ?? 'Teknisi belum terdaftar' }}</td>
							</tr>
							<tr>
