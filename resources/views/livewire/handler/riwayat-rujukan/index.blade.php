...
<div class="flex flex-col gap-5">
    ...
    {{-- ───── Filter Card ───── --}}
    <div class="dark:bg-dark-primary rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5 dark:border-zinc-800">
    ...
    <div class="grid grid-cols-2 gap-4 sm:gap-5 lg:grid-cols-4">
        ...
        {{-- Total Rujukan --}}
        <div class="dark:bg-dark-primary flex items-center gap-3.5 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5 dark:border-zinc-800">
            ...
            <div class="min-w-0">
                ...
                <p class="text-2xl font-extrabold leading-tight text-zinc-900 dark:text-white">{{ number_format($statsTotal) }}</p>
        ...
        {{-- Selesai --}}
        <div class="dark:bg-dark-primary flex items-center gap-3.5 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5 dark:border-zinc-800">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-950/40">
            ...
            <div class="min-w-0">
                ...
                <p class="text-2xl font-extrabold leading-tight text-zinc-900 dark:text-white">{{ number_format($statsSelesai) }}</p>
                @if ($statsTotal > 0)
                    <p class="text-[11px] text-zinc-400">{{ number_format($statsSelesai / $statsTotal * 100, 2) }}% dari total</p>
                @else
        ...
        {{-- Proses --}}
        <div class="dark:bg-dark-primary flex items-center gap-3.5 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5 dark:border-zinc-800">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-amber-50 dark:bg-amber-950/40">
            ...
            <div class="min-w-0">
                ...
                <p class="text-2xl font-extrabold leading-tight text-zinc-900 dark:text-white">{{ number_format($statsProses) }}</p>
                @if ($statsTotal > 0)
                    <p class="text-[11px] text-zinc-400">{{ number_format($statsProses / $statsTotal * 100, 2) }}% dari total</p>
                @else
                    <p class="text-[11px] text-zinc-400">0% dari total</p>
                @endif
            </div>
        ...
        <div class="dark:bg-dark-primary flex items-center gap-3.5 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5 dark:border-zinc-800">
            ...
            <div class="min-w-0">
                ...
                <p class="text-2xl font-extrabold leading-tight text-zinc-900 dark:text-white">{{ number_format($statsDitolak) }}</p>
                @if ($statsTotal > 0)
                    <p class="text-[11px] text-zinc-400">{{ number_format($statsDitolak / $statsTotal * 100, 2) }}% dari total</p>
                @else
                    <p class="text-[11px] text-zinc-400">0% dari total</p>
                @endif
            </div>
    ...
    <div class="dark:bg-dark-primary rounded-2xl border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-800">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-zinc-700 dark:text-zinc-300">
                <thead class="border-b border-zinc-200/80 bg-zinc-50/80 text-[11px] font-semibold uppercase tracking-wider text-zinc-500 dark:border-zinc-700/80 dark:bg-zinc-800/80 dark:text-zinc-400">
                    <tr>
                    ...
                    @forelse ($rujukanList as $item)
                        @php
                            $statusColor = match ($item->status) {
                                \App\Enums\StatusRujukan::Selesai   => 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300',
                                \App\Enums\StatusRujukan::Disetujui => 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
                                \App\Enums\StatusRujukan::Ditolak   => 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
                        ...
                        <tr class="transition-colors hover:bg-zinc-50/80 dark:hover:bg-zinc-800/50">
                            ...
                            <td class="px-4 py-3.5">
                                <p class="font-semibold text-zinc-900 dark:text-white">{{ $item->pasien?->nama ?? '-' }}</p>
                            ...
                            <td class="px-4 py-3.5 text-center">
                                <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold {{ $statusColor }}">
                                    {{ $item->status->label() }}
                                </span>
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                <a
                                    href="{{ route('rujukan.show', $item->id_rujukan) }}"
                                    wire:navigate
                        ...
                        <tr>
                            <td colspan="10" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="font-semibold text-zinc-500 dark:text-zinc-400">Tidak ada data rujukan</p>
        ...
        @if ($rujukanList->hasPages() || $rujukanList->total() > 0)
            <div class="flex flex-col items-center justify-between gap-3 border-t border-zinc-200/60 px-4 py-3.5 sm:flex-row dark:border-zinc-800/80">

                {{-- Info --}}
                <p class="text-xs text-zinc-400">
                    Menampilkan
                    <span class="font-semibold text-zinc-700 dark:text-zinc-300">{{ $rujukanList->firstItem() ?? 0 }} – {{ $rujukanList->lastItem() ?? 0 }}</span>
                    dari
                ...
                @if ($rujukanList->hasPages())
                    <div class="flex items-center gap-1">
                        ...
                        @if ($rujukanList->onFirstPage())
                            <span class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200/80 text-zinc-300 dark:border-zinc-700/80 dark:text-zinc-600">
                                <x-icons.chevron-left class="h-3.5 w-3.5" />
                            </span>
                        @else
                            <button wire:click="previousPage" class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200/80 text-zinc-600 transition hover:bg-zinc-100 dark:border-zinc-700/80 dark:text-zinc-300 dark:hover:bg-zinc-800">
                                <x-icons.chevron-left class="h-3.5 w-3.5" />
                            </button>
                        @endif
                        ...
                        @foreach ($rujukanList->getUrlRange(max(1, $rujukanList->currentPage() - 2), min($rujukanList->lastPage(), $rujukanList->currentPage() + 2)) as $page => $url)
                            @if ($page === $rujukanList->currentPage())
                                <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-xs font-bold text-white shadow-sm">
                                    {{ $page }}
                                </span>
                            @else
                                <button wire:click="gotoPage({{ $page }})" class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200/80 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100 dark:border-zinc-700/80 dark:text-zinc-300 dark:hover:bg-zinc-800">
                                    {{ $page }}
                                </button>
                            @endif
                        @endforeach
                        ...
                        @if ($rujukanList->currentPage() + 2 < $rujukanList->lastPage())
                            ...
                            <button wire:click="gotoPage({{ $rujukanList->lastPage() }})" class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200/80 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100 dark:border-zinc-700/80 dark:text-zinc-300 dark:hover:bg-zinc-800">
                        ...
                        @if ($rujukanList->hasMorePages())
                            <button wire:click="nextPage" class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200/80 text-zinc-600 transition hover:bg-zinc-100 dark:border-zinc-700/80 dark:text-zinc-300 dark:hover:bg-zinc-800">
