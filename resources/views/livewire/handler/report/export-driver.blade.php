...
<div class="rounded-xl border border-zinc-200 p-4 shadow-md dark:border-zinc-800 md:p-6"
    ...
    <form wire:submit="export" class="flex flex-col gap-4 md:gap-6">
        ...
        <div>
            ...
            <p class="text-xs text-zinc-500 dark:text-zinc-400">Sesuaikan rentang tanggal, tipe tagihan, tipe kunjungan,
                driver, status validasi, status pengantaran, dan filter petugas untuk mengekspor data Driver.</p>
        </div>
                ...
                <div x-show="open"
                    ...
                    @foreach ($this->driverSearchResults as $d)
                        <button type="button"
                            ...
                            <span
                                class="mr-2 font-bold text-blue-600 dark:text-blue-400">#{{ $d->kode_pegawai }}</span>
                            <span>{{ $d->name }}</span>
                            @if (!$d->is_active)
                                <span
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach ($selectedDrivers as $d)
                        <span
                            ...
                            <span>#{{ $d['kode_pegawai'] }} - {{ $d['name'] }}@if (!($d['is_active'] ?? true))
        ...
        <div class="relative" x-data="{ open: true }" @click.outside="open = false">
            ...
            <div class="relative flex items-center">
                <input id="assigner_search" type="text" wire:model.live="assignerSearchQuery"
                ...
                <div x-show="open"
                    ...
                    @foreach ($this->assignerSearchResults as $a)
                        <button type="button"
                            ...
                            @if ($a->kode_pegawai)
                                <span
                                    class="mr-2 font-bold text-blue-600 dark:text-blue-400">#{{ $a->kode_pegawai }}</span>
                            @endif
                            <span>{{ $a->name }}</span>
                            @if (!$a->is_active)
                                <span
            ...
            @if (!empty($selectedAssigners))
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach ($selectedAssigners as $a)
                        <span
                            class="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700 dark:border-blue-800/60 dark:bg-blue-900/30 dark:text-blue-300">
                            <span>{{ $a['name'] }}@if (!($a['is_active'] ?? true))
                                    <span class="ml-1 font-medium text-red-500 dark:text-red-400">(nonaktif)</span>
                                @endif
                            </span>
