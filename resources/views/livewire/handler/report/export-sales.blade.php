...
<div class="rounded-xl border border-zinc-200 p-4 shadow-md dark:border-zinc-800 md:p-6"
    ...
    <form wire:submit="export" class="flex flex-col gap-4 md:gap-6">
        ...
        <div>
            ...
            <p class="text-xs text-zinc-500 dark:text-zinc-400">Sesuaikan rentang tanggal, sales, status validasi, status
                order customer, dan filter petugas untuk mengekspor data Sales.</p>
        </div>
        ...
        {{-- Sales Multiselect (Autocomplete) --}}
        <div class="relative" x-data="{ open: true }" @click.outside="open = false">
            <label class="mb-2 block text-sm font-bold text-zinc-900 dark:text-white" for="sales_search">
                Filter Sales (Multiselect)
            </label>
            <div class="relative flex items-center">
                <input id="sales_search" type="text" wire:model.live="salesSearchQuery" @focus="open = true"
                    placeholder="Ketik nama atau kode sales untuk menyaring..."
            ...
            @if (!empty($this->salesSearchResults))
                <div x-show="open"
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach ($selectedSales as $s)
                        <span
                            ...
                            <span>#{{ $s['kode_pegawai'] }} - {{ $s['name'] }}@if (!($s['is_active'] ?? true))
        ...
        {{-- Validator Multiselect (Autocomplete) --}}
        <div class="relative" x-data="{ open: true }" @click.outside="open = false">
            <label class="mb-2 block text-sm font-bold text-zinc-900 dark:text-white" for="validator_search">
                Filter Divalidasi Oleh (Multiselect)
            </label>
            <div class="relative flex items-center">
                <input id="validator_search" type="text" wire:model.live="validatorSearchQuery"
                    @focus="open = true" placeholder="Ketik nama atau kode petugas untuk menyaring..."
                    class="block w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-10 pr-4 text-sm font-medium text-zinc-900 transition-all focus:border-blue-500 focus:ring-blue-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-white" />
                <div class="pointer-events-none absolute left-3 flex items-center text-zinc-400 dark:text-zinc-500">
                    <x-icons.search class="h-5 w-5" />
                </div>
            ...
            {{-- Autocomplete Dropdown --}}
            @if (!empty($this->validatorSearchResults))
                <div x-show="open"
                    class="absolute left-0 right-0 z-[150] mt-1 max-h-60 overflow-y-auto rounded-xl border border-zinc-200 bg-white py-1 shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
                    @foreach ($this->validatorSearchResults as $v)
                        <button type="button"
                            ...
                            @if ($v->kode_pegawai)
                                <span
                                    class="mr-2 font-bold text-blue-600 dark:text-blue-400">#{{ $v->kode_pegawai }}</span>
                            @endif
                            <span>{{ $v->name }}</span>
                            @if (!$v->is_active)
                                <span
            ...
            {{-- Selected Tags --}}
            @if (!empty($selectedValidators))
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach ($selectedValidators as $v)
                        <span
                            class="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700 dark:border-blue-800/60 dark:bg-blue-900/30 dark:text-blue-300">
                            <span>{{ $v['name'] }}@if (!($v['is_active'] ?? true))
                                    <span class="ml-1 font-medium text-red-500 dark:text-red-400">(nonaktif)</span>
                                @endif
                            </span>
