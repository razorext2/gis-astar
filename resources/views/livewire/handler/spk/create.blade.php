{{-- Goal: Create SPK with custom item parameters, Livewire: App\Livewire\Handler\Spk\Create, Alpine: focusNamaCustomer(), show-pdf-modal --}}
    ...
    <form class="grid gap-4" method="POST" wire:submit.prevent="store">
                                        ...
                                        <td class="px-4 py-4 align-top">
                                            <div class="flex items-center justify-center gap-2">
                                                <x-button.secondary type="button"
                                                    id="edit-barang-{{ $index }}"
                                                    wire:click="editBarang({{ $index }})" title="Edit"
                                                    :iconOnly="true" class="!h-7 !w-7 !p-0">
                                                    <x-icons.pen class="h-4 w-4" />
                                                </x-button.secondary>
                                                <x-button.danger type="button" id="hapus-barang-{{ $index }}"
                                                    wire:click="hapusBarang({{ $index }})" title="Hapus"
                                                    :iconOnly="true" class="!h-7 !w-7 !p-0">
        ...
        <div class="flex w-full items-center justify-end gap-3 rounded-xl border border-zinc-200 p-4 shadow-md dark:border-zinc-800 lg:p-6"
            x-bind:class="dynamicBg ?
                'bg-glass-light dark:bg-glass-dark border-glass-border-light dark:border-glass-border-dark backdrop-blur-md shadow-sm' :
                'bg-white dark:bg-dark-primary border-zinc-200 dark:border-zinc-800 shadow-sm'">
            <x-button.secondary id="summary-button" wire:click="summary" type="button" wire:loading.attr="disabled"
                wire:target="summary">
                <x-slot name="icon">
