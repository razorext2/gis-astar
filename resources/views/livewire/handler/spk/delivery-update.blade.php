<div class="flex flex-col gap-2 p-4 lg:gap-4 lg:p-0">
    ...
    <x-utils.accordion-item id="accordion-packing-form" title="Tambah Riwayat Pengiriman"
        description="Klik untuk membuka form pengiriman" iconColor="primary" :expanded="false">
        <x-slot:icon>
        ...
        <form wire:submit.prevent="store" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="col-span-2">
                <x-input.select id="via" name="via" :labels="true" :textLabel="'Tipe Pengiriman'" :defaultOption="'Pilih tipe pengiriman'"
                    :options="[
                        'laut' => 'Laut / Container',
                        'darat' => 'Darat / Truck',
            ...
            @if (isset($spk_data->production->packing_list) && $form->via !== 'supir')
                <div class="col-span-2">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @empty
                            <div
                                class="flex items-center gap-2 rounded-xl border border-dashed border-zinc-200 p-3 dark:border-zinc-800">
                                <x-icons.exclamation-circle class="h-4 w-4 shrink-0 text-red-500" />
                                <span class="text-xs font-medium italic text-red-500">Tidak ada barang dalam packing
                                    list.</span>
                            </div>
                        @endforelse
                    </div>
            ...
            @if ($form->via === 'laut')
                ...
                <div class="col-span-2 lg:col-span-1">
                    <x-input.basic id="no_container" name="no_container" placeholder="Masukkan nomor container.."
                        wire:model="form.no_container">
                        No. Container
                    </x-input.basic>
                ...
                @if ($form->via === 'supir')
                    <div class="col-span-2">
                        <div class="flex items-end gap-3">
                            <div class="grow">
                                <x-input.basic type="text" id="nomor_sr" name="nomor_sr" wire:model="form.nomor_sr"
                                    placeholder="SR-XXXXXXXX">
                                    Nomor SR
                                    <span class="font-normal text-zinc-500">(SPK:
                    ...
                    <div class="col-span-2 lg:col-span-1">
                        <x-input.basic id="search_supir" name="search_supir"
                            placeholder="Cari nama atau kode jari supir.." wire:model.live="search_supir">
                            Cari Supir
                        </x-input.basic>
                        ...
                        @if (count($drivers))
                            <div class="mt-2 overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-800">
                                @foreach ($drivers as $driver)
                                    <button type="button"
                                        class="{{ !$loop->last ? 'border-b border-zinc-200 dark:border-zinc-800' : '' }} flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                                        wire:click="selectDriver('{{ $driver->kode_pegawai }}', '{{ $driver->name }}')">
                                        <div
                                            class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-zinc-100 text-xs font-bold text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
                                            {{ strtoupper(substr($driver->name, 0, 1)) }}
                                        </div>
                                        <div>
                                            <p class="font-semibold text-zinc-900 dark:text-white">{{ $driver->name }}
                                            </p>
            ...
            {{-- Actions --}}
            <div
                class="col-span-2 flex items-center justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-800">
