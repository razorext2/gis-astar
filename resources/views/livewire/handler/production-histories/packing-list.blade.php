...
<div class="flex flex-col gap-6" x-data="{ pdfUrl: null }" x-on:show-detail-modal.window="pdfUrl = $event.detail.url">
    ...
    <x-utils.accordion-item id="accordion-packing-form" title="Tambah Packing List?"
        description="Klik untuk membuat daftar packing baru untuk pengiriman ini" iconColor="green" :expanded="false">
        <x-slot:icon>
        ...
        <form class="flex flex-col gap-4" wire:submit.prevent="store" method="post">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex gap-x-4">
                    <div class="w-full">
                        <x-input.basic id="nama_barang" name="nama_barang" :labels="true" :type="'text'"
                            wire:model="itemForm.nama_barang" placeholder="Input nama barang/produk yang dipesan...">
                            Nama Barang
                        </x-input.basic>
                    ...
                    <div class="flex w-fit gap-x-4">
                        <div class="w-full">
                            <x-input.basic id="qty_barang" name="qty_barang" :labels="true" type="number"
                                min="1" wire:model="itemForm.qty_barang" placeholder="Input jumlah barang...">
                                Qty / Jlh
                            </x-input.basic>
                        ...
                        <div class="w-full">
                            <x-input.select id="satuan_barang" name="satuan_barang" wire:model="itemForm.satuan_barang"
                ...
                <div class="flex flex-col gap-4" x-show="$wire.itemForm.cara_input === 'manual'" x-transition x-cloak>
                    {{-- daftar sparepart --}}
                    <div class="flex flex-col gap-y-4">
                        <h4 class="block text-sm font-bold text-zinc-900 dark:text-white">Daftar Part</h4>
                        ...
                        <div class="flex items-start gap-4">
                            <div class="grow">
                                <x-input.basic id="nama_part" name="nama_part" :labels="true" :type="'text'"
                                    wire:model="partForm.nama_part" placeholder="Ketik nama part/sparepart yang dibawa">
                                    Nama Part
                                </x-input.basic>
                            ...
                            <div class="w-24">
                                <x-input.basic id="qty" name="qty" :labels="true" :type="'number'"
                                    placeholder="Qty" wire:model="partForm.qty">
                                    Qty
                                </x-input.basic>
                            ...
                            <div x-show="$wire.partForm.pack === 'Pack'" x-transition x-cloak class="grow">
                ...
                <div x-show="$wire.itemForm.cara_input === 'upload'" x-transition x-cloak
                    class="col-span-2 grid w-full grid-cols-1 gap-4 rounded-xl border border-zinc-200 p-4 dark:border-zinc-800 lg:p-6">
                    <div x-show="$wire.docForm.new_attachments.length > 0">
                        <span class="mb-2 block text-sm font-bold text-zinc-900 dark:text-white">
                            Daftar Lampiran
                        </span>
    ...
    <x-modal.base-modal show="showDetailModal" title="Pratinjau Packing List"
        subtitle="Dokumen Produksi & Distribusi" maxWidth="5xl">
        <x-slot name="icon">
