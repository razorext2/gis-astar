...
<div>
    <x-modal.base-modal
        show="showModal"
        title="Approve Laporan Produksi"
        subtitle="Pilih laporan yang akan disetujui"
        ...
        @else
            ...
            <div class="mb-4 flex items-center justify-between rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-white/5">
                ...
                @if (count($selectedIds) > 0)
                    <span class="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-bold text-green-700 dark:bg-green-900/30 dark:text-green-400">
            ...
            <div class="overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-800">
                {{-- Select All Header --}}
                <div class="flex items-center gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-800/50">
                    <input
                        type="checkbox"
                        id="select-all-histories"
                    ...
                    >
                    <label for="select-all-histories" class="cursor-pointer text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                ...
                <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @foreach ($this->histories as $history)
                        ...
                        @endphp
                        <label
                            for="history-{{ $history->id }}"
                            wire:key="bulk-history-{{ $history->id }}"
                            class="flex cursor-pointer items-start gap-3 px-4 py-3.5 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/40 {{ $isSelected ? 'bg-green-50/50 dark:bg-green-900/10' : '' }}"
                        >
                            <input
                                type="checkbox"
                            ...
                            <div class="flex min-w-0 flex-1 flex-col gap-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-600 ring-1 ring-blue-600/10 dark:bg-blue-900/30 dark:text-blue-400">
                                        {{ $statusLabel }}
                                    </span>
                                </div>
                                ...
                                <div class="flex items-center gap-3 text-[10px] text-zinc-400 italic">
                                    <span>Oleh: <span class="font-medium not-italic text-zinc-600 dark:text-zinc-400">{{ $history->addedBy?->name ?? 'Sistem' }}</span></span>
                                    <span>·</span>
