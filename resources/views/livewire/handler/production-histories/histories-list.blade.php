...
<div class="flex w-full flex-col gap-6">
    ...
    <div class="flex items-center justify-between border-b border-zinc-100 pb-4 dark:border-zinc-800">
        ...
        <div class="flex items-center gap-2">
            ...
            <x-input.select class="!py-1 text-xs" name="status" id="status" :labels="false" :defaultOption="'Semua Status'"
                :options="[0 => 'Diajukan', 1 => 'Disetujui', 2 => 'Ditolak', 3 => 'Semua']" wire:model.live.debounce.500ms="status_validasi" />
        </div>
    </div>

    {{-- Timeline List --}}
    <div
        class="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:-translate-x-px before:bg-gradient-to-b before:from-transparent before:via-zinc-200 before:to-transparent dark:before:via-zinc-800 md:before:ml-[5.5rem]">
        @forelse ($data as $row)
            <div class="relative flex items-start gap-4 md:gap-8" wire:key="history-{{ $row->id }}">
                ...
                {{-- Timeline Dot --}}
                <div
                    class="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-4 border-white bg-zinc-100 text-zinc-500 shadow-sm dark:border-zinc-900 dark:bg-zinc-800 dark:text-zinc-400">
                    @php
                ...
                <div class="flex flex-1 flex-col gap-3 rounded-xl border border-zinc-100 p-4 shadow-sm dark:border-zinc-800"
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <div class="flex flex-col gap-1">
                            <div class="flex items-center gap-2">
                                ...
                                @endphp
                                <span
                                    class="{{ $badgeClasses }} rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ring-1">
                        ...
                        <span
                            class="rounded-full bg-blue-50 px-2.5 py-0.5 text-[10px] font-bold text-blue-600 ring-1 ring-blue-600/10 dark:bg-blue-900/30 dark:text-blue-400">
                    ...
                    @if (isset($row->documentations) && count($row->documentations) > 0)
                        <div class="scrollbar-hide mt-2 flex gap-3 overflow-x-auto pb-2">
                            @foreach ($row->documentations as $img)
                                <div
                                    class="group relative h-20 w-20 shrink-0 overflow-hidden rounded-lg ring-1 ring-zinc-200 dark:ring-zinc-700">
                                    <img class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110"
                                        ...
                                        src="{{ asset('storage/' . $img['path_file']) }}" alt="Doc"
                                        loading="lazy">
                    ...
                    {{-- Footer: Meta & Actions --}}
                    <div
                        class="mt-2 flex items-center justify-between border-t border-zinc-100 pt-3 dark:border-zinc-800">
                        <div class="flex items-center gap-x-2 text-[10px] italic text-zinc-400">
                            <span>Oleh: <span
                                    class="font-medium text-zinc-600 dark:text-zinc-400">{{ $row->addedBy->name ?? 'Sistem' }}</span></span>
                            @if ($row->addedBy)
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @can('produksi-edit')
                                <a href="{{ route('production.history.add', ['id' => $this->id, 'history_id' => $row->id]) }}"
                                    wire:navigate class="text-xs font-medium text-zinc-500 hover:text-blue-600">Ubah</a>
                            @endcan
