		...
		@if ($showModal)
			...
			<div
				...
				@if ($step == 1)
									...
									@if ($data->photoCollectRelasi && $data->photoCollectRelasi->count() > 0)
										@foreach ($data->photoCollectRelasi as $photo)
											<div class="relative me-2 flex-none items-center gap-4 rounded-xl p-2">
												<img
													class="h-52 w-52 rounded-xl object-cover transition duration-300 ease-in-out hover:scale-150 lg:hover:scale-[2]"
					...
					@else
						<p class="text-gray-800 dark:text-gray-200">Apakah anda yakin ingin mengonfirmasi laporan ini?</p>
					@endif
				...
				@elseif($step == 2)
					<form wire:submit="confirmQuestionnaire" enctype="multipart/form-data">
						@csrf
						<h2 class="mb-2 text-center text-2xl font-semibold text-gray-900 dark:text-white lg:text-3xl">Kuisioner</h2>
						<div class="flex max-h-96 w-full flex-col gap-2 overflow-y-auto text-gray-800 dark:text-white md:gap-4">
							<p>Jawab pertanyaan - pertanyaan berikut sebelum konfirmasi laporan.</p>

							<div>
								<x-input.basic placeholder="Cth: Bp. Bintan" id="customer_name" name="customer_name" wire:model="customer_name"
									required>
									Nama Customer
								</x-input.basic>
							...
							<div>
								...
								<div class="mb-2 ms-1 flex items-center">
									<input id="option-1" type="radio" name="customer_make_order" wire:model="customer_make_order" value="1"
										...
										checked>
									<label for="option-1" class="ms-2 block text-sm font-medium text-gray-900 dark:text-gray-300">
										Ya
									</label>
								</div>

								<div class="ms-1 flex items-center">
									<input id="option-2" type="radio" name="customer_make_order" wire:model="customer_make_order" value="0"
										class="h-4 w-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:border-gray-600 dark:bg-gray-700 dark:focus:bg-blue-600 dark:focus:ring-blue-600">
									<label for="option-2" class="ms-2 block text-sm font-medium text-gray-900 dark:text-gray-300">
										Tidak
									</label>
								</div>
							...
							<div>
								...
								<p class="mb-2 text-wrap text-xs text-gray-500 dark:text-gray-400">
									Jika customer order, sebutkan apa saja yg diorder. Jika tidak, jelaskan alasan customer tidak order
								</p>
								<x-input.textarea :labels="false" id="order_notes" name="order_notes" wire:model="order_notes" />
							...
							<div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
								...
								<label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">Bukti Followup
									Customer</label>
								...
								@if ($proof_pic)
									...
									<button type="button" wire:click="$cancelUpload('proof_pic')">Cancel Upload</button>
								@endif
						...
						<div class="mt-4 flex justify-end space-x-2">
							<x-button.success id="confirmQuestionnaireBtn" type="submit">
								...
								<span wire:loading.remove wire:target="proof_pic, confirmQuestionnaireBtn">Proses Konfirmasi</span>
							</x-button.success>
							<x-button.primary id="cancelBtn" wire:click="resetModal">Batal</x-button.primary>
						</div>
					</form>
				@elseif($step == 3)
					<h2 class="mb-4 text-center text-2xl font-semibold text-gray-900 dark:text-white lg:text-3xl">Alasan Penolakan
					</h2>
					<div class="flex w-full flex-col gap-2 text-gray-800 dark:text-white md:gap-4">
						...
						<x-input.textarea :labels="false" id="rejectionReason" name="rejectionReason" wire:model="rejectionReason" />
					</div>
