<div class="my-4 flex flex-col gap-4">
    ...
    <div class="flex items-center gap-3">
        <x-button.primary wire:click="fetchResi" wire:loading.attr="disabled" wire:target="fetchResi"
            class="group relative w-fit overflow-hidden text-sm">
            <x-slot name="icon">
                <x-icons.angle-right wire:loading.remove wire:target="fetchResi" class="icon h-5 w-5" />
                <x-icons.loading wire:loading wire:target="fetchResi" class="h-4 w-4 animate-spin" />
            </x-slot>

            <span wire:loading.remove wire:target="fetchResi">Cek Status Resi</span>
            <span wire:loading wire:target="fetchResi">Memproses...</span>
        </x-button.primary>
    ...
    @if ($error)
        <div class="flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-600 dark:border-red-900/30 dark:bg-red-900/10 dark:text-red-400"
            ...
            <x-button.secondary class="!bg-transparent !p-1 !ring-0 hover:!bg-red-100 dark:hover:!bg-red-900/20"
                @click="show = false">
                <x-slot name="icon">
    ...
    @if ($data)
        <div class="flex flex-col gap-6 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-300 dark:border-zinc-800 dark:shadow-none lg:p-6"
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div class="dark: dark:hover: group h-full rounded-2xl border border-zinc-200 bg-zinc-50/50 p-4 transition-all hover:border-red-500/30 hover:bg-white dark:border-white/5 dark:hover:border-red-500/20"
                    x-bind:class="dynamicBg ?
                        'bg-glass-light dark:bg-glass-dark border-glass-border-light dark:border-glass-border-dark backdrop-blur-md shadow-sm' :
                        'bg-white dark:bg-dark-primary border-zinc-200 dark:border-zinc-800 shadow-sm'">
                ...
                <div class="relative space-y-6 pl-6">
                    ...
                    @foreach ($data['data']['history'] as $history)
                        <div class="relative">
                            ...
                            <div class="dark: dark:hover: flex flex-col gap-2 rounded-xl border border-zinc-200 p-4 transition-all hover:border-red-200 hover:bg-white dark:border-zinc-800 dark:hover:border-red-900/30"
                                x-bind:class="dynamicBg ?
                                    'bg-glass-light dark:bg-glass-dark border-glass-border-light dark:border-glass-border-dark backdrop-blur-md shadow-sm' :
                                    'bg-white dark:bg-dark-primary border-zinc-200 dark:border-zinc-800 shadow-sm'">
