<div class="flex w-full flex-col gap-4">
    ...
    <div wire:key="create-participant-form"
        ...
        <div class="mb-6 flex items-center justify-between border-b border-zinc-100 pb-5 dark:border-zinc-800">
            <div>
            ...
            <button wire:click="$set('showCreateForm', false)" class="text-zinc-400 hover:text-zinc-900 dark:hover:text-white">
        ...
        <form wire:submit.prevent="store" class="space-y-6">
            ...
            <div class="space-y-4">
                <div class="w-full">
                    <x-input.basic id="search-user" name="search_user" wire:model.live.throttle.100ms="search" placeholder="Ketik nama karyawan..." :labels="'Cari User'">
                        Nama Partisipan
                    </x-input.basic>

                    @error('user_id')
                        <p class="mt-2 text-xs font-bold text-red-500 italic">{{ $message }}</p>
                    @enderror
                </div>

                {{-- Result List --}}
                <div class="max-h-60 space-y-2 overflow-y-auto rounded-2xl border border-zinc-100 bg-zinc-50/50 p-2 dark:border-zinc-800 dark:bg-zinc-950/20">
                    @forelse ($users as $user)
                        <label for="user-option-{{ $user->id }}" 
                            class="flex cursor-pointer items-center justify-between rounded-xl border border-transparent p-3 transition-all hover:bg-white hover:shadow-sm dark:hover:bg-zinc-800 {{ $user_id == $user->id ? 'bg-white border-red-500/30 shadow-md dark:bg-zinc-800 dark:border-red-600/50' : '' }}">
                            <div class="flex items-center gap-3">
                                <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-200 text-xs font-bold text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400">
                                    {{ strtoupper(substr($user->name, 0, 2)) }}
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-sm font-bold text-zinc-900 dark:text-white">{{ $user->name }}</span>
                                    <span class="text-[10px] font-black tracking-widest text-zinc-400 uppercase">{{ $user->kode_pegawai ?? '-' }}</span>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center py-10 opacity-40">
                            <svg class="h-10 w-10 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
