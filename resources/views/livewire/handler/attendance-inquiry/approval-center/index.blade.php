...
<div class="rounded-xl border border-zinc-200 p-4 shadow-sm dark:border-zinc-800 md:p-6 lg:p-8"
    ...
    <div class="border-b border-zinc-200 pb-4 dark:border-zinc-800">
        ...
        <p class="text-sm text-zinc-500 dark:text-zinc-400">Verifikasi dan setujui permintaan koreksi/input manual absensi karyawan.</p>
    </div>
        ...
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
            <li class="mr-2">
                <button wire:click="setTab('pending')"
                    class="inline-block p-4 border-b-2 rounded-t-lg transition-all duration-150 {{ $activeTab === 'pending' ? 'border-blue-600 text-blue-600 font-bold dark:border-blue-500 dark:text-blue-500' : 'border-transparent text-zinc-500 hover:text-zinc-600 hover:border-zinc-300 dark:text-zinc-400' }}">
                    Menunggu Verifikasi
                </button>
            </li>
            <li class="mr-2">
                <button wire:click="setTab('approved')"
                    class="inline-block p-4 border-b-2 rounded-t-lg transition-all duration-150 {{ $activeTab === 'approved' ? 'border-blue-600 text-blue-600 font-bold dark:border-blue-500 dark:text-blue-500' : 'border-transparent text-zinc-500 hover:text-zinc-600 hover:border-zinc-300 dark:text-zinc-400' }}">
                    Disetujui
                </button>
            </li>
            <li class="mr-2">
                <button wire:click="setTab('rejected')"
                    class="inline-block p-4 border-b-2 rounded-t-lg transition-all duration-150 {{ $activeTab === 'rejected' ? 'border-blue-600 text-blue-600 font-bold dark:border-blue-500 dark:text-blue-500' : 'border-transparent text-zinc-500 hover:text-zinc-600 hover:border-zinc-300 dark:text-zinc-400' }}">
                    Ditolak
                </button>
            </li>
        </ul>
                        ...
                        <tr wire:key="approval-{{ $inquiry->id }}" class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="font-bold text-zinc-900 dark:text-white">{{ $inquiry->user->name ?? 'Karyawan' }}</span>
                                    <span class="text-xs text-zinc-500 dark:text-zinc-400">ID: {{ $inquiry->kode_pegawai }}</span>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <a href="{{ route('attendance-inquiry.approval-center.show', $inquiry->id) }}" wire:navigate
