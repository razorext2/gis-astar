...
<div class="space-y-6" x-data="{ showRejectForm: false }">
    ...
    <div class="flex items-center gap-4 rounded-xl border border-zinc-200 bg-white/60 p-4 shadow-sm backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/60">
        <a href="{{ route('attendance-inquiry.approval-center.index') }}" wire:navigate class="rounded-lg border border-zinc-200 p-2 text-zinc-600 hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-400 dark:hover:bg-zinc-800">
            <x-icons.arrow-left class="h-5 w-5" />
        </a>
        <div class="flex-1">
            <h2 class="text-xl font-bold text-zinc-900 dark:text-white">Tinjau Laporan Absensi</h2>
            <p class="text-xs text-zinc-500 dark:text-zinc-400">Diajukan oleh {{ $inquiry->user->name ?? 'Karyawan' }} (ID: {{ $inquiry->kode_pegawai }})</p>
        </div>
        <div>
            @php
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if ($inquiry->status === 'pending')
                <div class="rounded-xl border border-zinc-200 bg-white/60 p-5 dark:border-zinc-800 dark:bg-zinc-900/60 space-y-4">
                    ...
                    <div x-show="showRejectForm" x-collapse class="mt-4 border-t border-zinc-100 pt-4 dark:border-zinc-800 space-y-3">
                        <label for="rejection_reason" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Alasan Penolakan <span class="text-red-500">*</span></label>
                        <textarea id="rejection_reason" wire:model.live="rejection_reason" rows="3" placeholder="Masukkan alasan mengapa pengajuan ini ditolak..."
                        ...
                        @error('rejection_reason') <span class="text-xs text-red-500 block">{{ $message }}</span> @enderror
                        
                        <div class="flex justify-end gap-2">
                            <button type="button" @click="showRejectForm = false" class="text-sm font-semibold text-zinc-500 hover:text-zinc-600 dark:text-zinc-400">Batal</button>
                            <button type="button" wire:click="reject" class="text-sm font-bold text-red-600 hover:text-red-500 dark:text-red-400">Kirim Penolakan</button>
                        </div>
                    </div>
                </div>
            @endif
