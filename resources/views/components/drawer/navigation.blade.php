...
<div {{ $attributes }} class="fixed bottom-3 left-1/2 z-[160] w-[92vw] max-w-sm -translate-x-1/2 md:hidden">
    ...
    <div x-bind:class="dynamicBg
        ...
        <div class="mx-auto grid h-full max-w-sm grid-cols-5 px-3">
            ...
            <x-drawer.button href="{{ Route::has('pasien.index') ? route('pasien.index') : '#' }}" :label="'Pasien'" :active="Route::is('pasien.*')">
                <x-icons.user
                    class="{{ Route::is('pasien.*') ? 'text-red-600 dark:text-red-500' : 'text-zinc-400 dark:text-zinc-500' }} h-6 w-6 transition-all duration-300 group-hover:text-red-500 dark:group-hover:text-red-400" />
            </x-drawer.button>
            ...
            <x-drawer.button href="{{ Route::has('peta-rute.index') ? route('peta-rute.index') : '#' }}" :label="'Peta'" :active="Route::is('peta-rute.*')">
                <x-icons.map
                    class="{{ Route::is('peta-rute.*') ? 'text-red-600 dark:text-red-500' : 'text-zinc-400 dark:text-zinc-500' }} h-6 w-6 transition-all duration-300 group-hover:text-red-500 dark:group-hover:text-red-400" />
            </x-drawer.button>
