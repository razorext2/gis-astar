    ...
    if ($user) {
        ...
        if ($assignedProductionCount > 0) {
            $popups[] = [
                ...
                'stack'   => $stackIndex++,
                'message' => 'tugas produksi menunggu <span class="font-bold text-zinc-900 dark:text-white">pengerjaan</span> Anda',
            ];
        }
    }
@endphp
...
@if (count($popups) > 0)
    <div x-data="{
        ...
        {{-- FAB Group --}}
        <div class="report-fab-group {{ $hasPendingLeave ? 'bottom-above-leave' : 'bottom-base' }} fixed right-4 z-50 h-11 w-11 md:right-8">
