			...
			<div class="lg:hidden" id="sub-navbar" x-data="{ open: false }">
				...
				<div class="mt-2 grid w-full gap-2 md:mt-4 md:gap-4" x-show="open" x-transition>
					<ul class="rounded-lg bg-white text-gray-700 shadow dark:bg-gray-800 dark:text-gray-200">
						...
						<li>
							<a
								class="{{ Route::is('driver.index') && Request::query('status') == 2 ? 'text-red-600 border-b border-gray-400' : 'text-gray-600 dark:text-gray-400' }} inline-block w-full rounded-lg border-none p-3 text-sm transition-all duration-300 ease-in-out will-change-transform hover:scale-105 hover:dark:bg-gray-500"
								href="{{ route('driver.index', ['status' => 2]) }}">Perlu Revisi</a>
						</li>
						<li>
							<a
								class="{{ Route::is('driver.index') && Request::query('status') == 1 ? 'text-red-600 border-b border-gray-400' : 'text-gray-600 dark:text-gray-400' }} inline-block w-full rounded-lg border-none p-3 text-sm transition-all duration-300 ease-in-out will-change-transform hover:scale-105 hover:dark:bg-gray-500"
								href="{{ route('driver.index', ['status' => 1]) }}">Disetujui</a>
						</li>
						<li>
							<a
								class="{{ Route::is('driver.index') && Request::query('status') == 3 ? 'text-red-600 border-b border-gray-400' : 'text-gray-600 dark:text-gray-400' }} inline-block w-full rounded-lg border-none p-3 text-sm transition-all duration-300 ease-in-out will-change-transform hover:scale-105 hover:dark:bg-gray-500"
								href="{{ route('driver.index', ['status' => 3]) }}">Ditolak</a>
						</li>
					</ul>
				</div>
