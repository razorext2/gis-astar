...
@section('content')
        ...
        <div class="w-full">
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="w-full md:col-span-2">
                    <form id="dateForm"
                        action="{{ route('routes.collector.detail', ['pegawai' => $pegawai->kode_pegawai]) }}" method="GET">
                        <x-dashboard.date-picker id="tanggal-rute-collector" name="date" form="dateForm"
                            :text="'Filter tanggal'" />
                    </form>
                </div>
                {{-- endsearch --}}
                            ...
                            @foreach ($report as $data)
                                <li class="relative mb-10 ms-8">
                                    ...
                                    <h3 class="mb-1 flex items-center text-sm text-gray-900 dark:text-white">
                                        ...
                                        @if ($data->status == 0)
                                            <span
                                                class="me-2 ms-3 rounded bg-yellow-100 p-0.5 text-sm font-medium text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300">No
                                                update yet</span>
                                    ...
                                    <span
                                        ...
                                        @if ($data->latitude && !$loop->first)
                                            @php
                                                ...
                                                $currentTime = Carbon\Carbon::parse($data->assign_at);
                                    ...
                                    <time
                                        ...
                                        Kolektor update:
                                        {{ \Carbon\Carbon::parse($data->assign_at)->locale('id')->isoFormat('DD MMM YYYY HH:mm:ss') }}
                                    </time>
                                </li>
                            @endforeach
                        @else
    ...
    <script type="module">
        ...
        document.getElementById('dateForm').addEventListener('submit', function(e) {
            const dateInput = document.getElementById('tanggal-rute-collector').value;
