		...
		<div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 dark:bg-[#18181b] dark:ring-gray-700 sm:p-6">
			<div class="max-w-xl">
				<header class="flex flex-row gap-x-3">
					<div class="max-w-xs">
						<x-button.link class="w-fit ring-1 ring-red-700 dark:bg-red-800 dark:text-white" href="{{ route('division.index') }}"
							wire:navigate>
							<x-slot name="icon">
				...
				<form class="mt-4" action="{{ route('division.update', $division) }}" method="POST">
					@csrf
					@method('put')
					<div class="mb-4 grid gap-6 sm:mb-5 sm:grid-cols-2 sm:gap-6">
						<div class="sm:col-span-2">
							<label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="kode_divisi">Kode Divisi</label>
							<input
								...
								id="kode_divisi" name="kode_divisi" type="text" value="{{ $division->kode_divisi }}" placeholder="Kode Divisi"
								required="">
						</div>
						<div class="w-full">
							<label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="divisi">Divisi</label>
							<input
								...
								id="divisi" name="divisi" type="text" value="{{ old('nama_divisi', $division->nama_divisi ?? '') }}"
