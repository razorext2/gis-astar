	...
	<div
		...
		<form method="POST" action="{{ route('event.store') }}" class="grid grid-cols-2 gap-4">
			...
			<div class="w-full">
				...
				<input
					class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
					type="date" id="end_date" required name="end_date" placeholder="Tanggal Selesai" value="{{ old('end_date') }}">
			...
			<div class="col-span-2 flex w-full flex-col">
				...
				<x-input.textarea id="description" class="@error('description') border-red-500 @enderror" required name="description"
					:labels="false" placeholder="Deskripsi event">{{ old('description') }}</x-input.textarea>
