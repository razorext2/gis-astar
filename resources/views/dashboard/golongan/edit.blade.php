...
@section('content')
	<div class="w-full">
		<form class="mt-4" action="{{ route('golongan.update', $golongan) }}" method="POST">
			@csrf
			@method('put')
						...
						<div class="max-w-xl">
							...
							<div class="my-4 grid gap-6 sm:gap-6 md:grid-cols-2">
								<div class="w-full">
									...
									<input
										...
										id="nama_golongan" name="nama_golongan" type="text"
										value="{{ old('nama_divisi', $golongan->nama_golongan ?? '') }}" placeholder="Nama Golongan" required="">
								</div>
								<div class="w-full">
									...
									<input
										...
										id="alias" name="alias" type="text" value="{{ old('alias', $golongan->alias ?? '') }}"
							...
							<div class="rounded-lg sm:my-4" id="jadwal">
								@php
									...
									// Create a keyed collection of jadwal for easy access
									$jadwal = $golongan->jadwalRelasi->keyBy('hari');
								@endphp
									...
									<div class="w-full border-b border-b-gray-500">
										<div class="grid gap-2 py-4 sm:flex md:gap-4">
											<div class="w-20 sm:flex-none">
												<h3 class="text-md mt-0 font-semibold text-gray-700 dark:text-white md:mt-9">{{ ucfirst($day) }}</h3>
											</div>

											<div class="w-full sm:flex-1">
												<label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
													for="jam_masuk_{{ strtolower($day) }}">Jam Masuk</label>
												<input
													class="focus:ring-primary-600 block w-full rounded-lg border border-gray-300 p-2.5 text-sm text-gray-900"
													id="jam_masuk_{{ strtolower($day) }}" name="jam_masuk_{{ strtolower($day) }}" type="time"
													value="{{ $jadwal[$day]->jam_masuk }}" required>
											</div>

											<div class="w-full sm:flex-1">
												<label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
													for="jam_keluar_{{ strtolower($day) }}">Jam Keluar</label>
												<input
													class="focus:ring-primary-600 block w-full rounded-lg border border-gray-300 p-2.5 text-sm text-gray-900"
													id="jam_keluar_{{ strtolower($day) }}" name="jam_keluar_{{ strtolower($day) }}" type="time"
													value="{{ $jadwal[$day]->jam_keluar }}" required>
											</div>
