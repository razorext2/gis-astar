...
trait HasLeaveRequestForm
{
    ...
    protected function checkDateOverlap(): void
    {
        ...
        if (Carbon::parse($this->start_date)->greaterThan(Carbon::parse($this->end_date))) {
        ...
        if ($overlap) {
            $from = Carbon::parse($overlap->start_date)->locale('id')->isoFormat('D MMM YYYY');
            $to = Carbon::parse($overlap->end_date)->locale('id')->isoFormat('D MMM YYYY');
    ...
    protected function checkMinAdvanceDays(): bool
    {
        ...
        $minStartDate = Carbon::today()->addDays($minDays);

        if ($this->start_date && Carbon::parse($this->start_date)->lessThan($minStartDate)) {
